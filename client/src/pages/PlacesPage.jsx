...
export default function PlacesPage (){
  ...
  function preInpnut (header, description) {
    return (
      <>
        {inputHeader(header)}
    ...
    return (
        <div>
          ...
          {action === 'new' &&(
            <div>
              <form>
                {preInpnut('Title','Title for your place. should be short and catchy as in advertisement')}
                <input type="text" value={title} onChange={ev => setTitle(ev.target.value)} placeholder="title, for example: My lovely apt"/>
                {preInpnut('Adress','Address to this place')}
                <input type="text" value={address} onChange={ev => setAddress(ev.target.value)} placeholder="adress" />
                {preInpnut('Photos','more = better')}
      ...
      <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        ...
        <label className="h-32 cursor-pointer flex items-center gap-1 justify-center border bg-transparent rounded-2xl p-2 text-2xl text-gray-600">
          ...
          Upload
                ...
                {preInpnut('Description','Description of the place')}
                <textarea value={description} onChange={ev => setDescription(ev.target.value)}/>
                {preInpnut('Perks','Select all the perks of your place')}
                <div className='grid mt-2 gap-2 grid-cols-2 md:grid-cols-3 lg:grid-cols-3'>
                  <Perks selected = {Perks} onChange= {setPerks}/>
                </div>
                {preInpnut('Extra info','House rules, etc')}
                
                <textarea value={extraInfo} onChange={ev => setExtraInfo(ev.target.value)}/>
                {preInpnut('Check in&out times','Add check in and out times, remember to have some time window for cleaning the room between guests')}
